...
<head>
  ...
  <style>
    * { box-sizing: border-box; }
    ...
    .sidebar { width: 240px; min-height: 100vh; background: #050811; border-left: 1px solid #1e293b; position: fixed; top: 0; right: 0; z-index: 40; display: flex; flex-direction: column; }
    ...
    .nav-btn { display: flex; align-items: center; gap: 12px; padding: 11px 16px; border-radius: 12px; font-size: 14px; font-weight: 700; color: #94a3b8; transition: all 0.2s; width: 100%; text-align: right; background: none; border: none; cursor: pointer; text-decoration: none; }
    ...
    .modal-box { background: #0F172A; border: 1px solid #1e293b; border-radius: 20px; width: 100%; max-width: 820px; max-height: 90vh; overflow-y: auto; box-shadow: 0 25px 60px rgba(0,0,0,0.6); }
...
<!-- Sidebar -->
<aside class="sidebar">
  <div style="padding: 20px 16px; border-bottom: 1px solid #1e293b; display: flex; align-items: center; gap: 12px;">
    ...
    <div>
      <div style="color: #fff; font-weight: 800; font-size: 15px;">برق سهيل</div>
      <div style="color: #64748b; font-size: 11px;">لوحة التحكم والمخزون</div>
    </div>
  </div>
  ...
  <div style="padding: 24px;">
    ...
    <div id="tab-products">
      ...
      <div class="glass-card" style="overflow: hidden;">
        <div style="display: grid; grid-template-columns: 60px 1fr 140px 90px 70px 70px 100px; gap: 12px; padding: 12px 18px; border-bottom: 1px solid #1e293b; font-size: 11px; font-weight: 800; color: #64748b; text-transform: uppercase;">
          <div>الصورة</div>
          <div>اسم الصنف / الرمز</div>
          <div>الفئة</div>
          <div style="text-align: center;">السعر</div>
          <div style="text-align: center;">خصم</div>
    ...
    <div id="tab-add" style="display: none; max-width: 760px; margin: 0 auto;">
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
          ...
          <div style="display: flex; flex-direction: column; gap: 14px;">
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px;">
              <div>
                <label style="display: block; font-size: 12px; font-weight: 700; color: #94a3b8; margin-bottom: 6px;">💰 السعر الحالي (ر.س) *</label>
                <input type="number" id="add-price" class="admin-input" placeholder="0" min="0" style="color: #00D2FF; font-weight: 800; font-family: monospace;">
              </div>
              <div>
                <label style="display: block; font-size: 12px; font-weight: 700; color: #94a3b8; margin-bottom: 6px;">🔖 السعر قبل التخفيض</label>
                <input type="number" id="add-old-price" class="admin-input" placeholder="اختياري" min="0" style="font-family: monospace;">
              </div>
            </div>
            ...
            <div>
              ...
              <textarea id="add-desc" class="admin-input" rows="4" style="resize: vertical;" placeholder="اكتب وصفاً أو مواصفات فنية..."></textarea>
            </div>
          </div>
        </div>
...
function loadData() {
  ...
  // 2. If empty, grab from INITIAL_PRODUCTS or INVENTORY_PRODUCTS from data.js
...
function resetToDefault() {
  if (!confirm('هل تريد استعادة بيانات الجرد الافتراضية كاملة؟')) return;
  if (typeof INITIAL_PRODUCTS !== 'undefined' && INITIAL_PRODUCTS.length > 0) {
    storeProducts = JSON.parse(JSON.stringify(INITIAL_PRODUCTS));
  } else if (typeof INVENTORY_PRODUCTS !== 'undefined' && INVENTORY_PRODUCTS.length > 0) {
    storeProducts = JSON.parse(JSON.stringify(INVENTORY_PRODUCTS));
...
function switchTab(tabId) {
  ['products', 'add', 'stats'].forEach(function(t) {
...
function renderProductsTable() {
  ...
  container.innerHTML = filtered.map(function(p) {
    ...
    return '<div style="display: grid; grid-template-columns: 60px 1fr 140px 90px 70px 70px 100px; gap: 12px; padding: 12px 18px; align-items: center; border-bottom: 1px solid rgba(30, 41, 59, 0.5); transition: background 0.15s;" onmouseover="this.style.background=\\'rgba(0,210,255,0.03)\\'" onmouseout="this.style.background=\\'transparent\\'">' +
      ...
      '<div style="text-align: center;"><div style="color: #00D2FF; font-size: 14px; font-weight: 800; font-family: monospace;">' + (p.price || 0).toLocaleString() + '</div><div style="color: #64748b; font-size: 10px;">ر.س</div></div>' +
...
function openEditModal(productId) {
  ...
  var modalHtml = '<div style="padding: 24px;">' +
    ...
    '<div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">' +
      ...
      '<div style="display: flex; flex-direction: column; gap: 14px;">' +
        '<div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px;">' +
          '<div><label style="display: block; font-size: 12px; font-weight: 700; color: #94a3b8; margin-bottom: 6px;">💰 السعر الحالي (ر.س)</label><input type="number" id="edit-price" class="admin-input" value="' + (p.price || 0) + '" min="0" style="color: #00D2FF; font-weight: 800; font-family: monospace;" oninput="updateDiscountCalc()"></div>' +
          '<div><label style="display: block; font-size: 12px; font-weight: 700; color: #94a3b8; margin-bottom: 6px;">🔖 السعر القديم (قبل الخصم)</label><input type="number" id="edit-old-price" class="admin-input" value="' + (p.oldPrice || '') + '" min="0" placeholder="اتركه فارغاً إن لم يوجد خصم" style="font-family: monospace;" oninput="updateDiscountCalc()"></div>' +
        '</div>' +
        ...
        '</div>' +
        '<div><label style="display: block; font-size: 12px; font-weight: 700; color: #94a3b8; margin-bottom: 6px;">📝 وصف الصنف</label><textarea id="edit-desc" class="admin-input" rows="4" style="resize: vertical;">' + (p.shortDesc || '') + '</textarea></div>' +
      '</div>' +
    '</div>' +
    '<div style="margin-top: 18px;">' +
...
function saveEditChanges() {
  ...
  var oldPrice = parseFloat(document.getElementById('edit-old-price').value) || 0;
...
function saveNewProduct() {
  ...
  var price = parseFloat(document.getElementById('add-price').value) || 0;
  if (!price) { showToast('❌ يجب تحديد سعر الصنف!'); return; }

  var cat = document.getElementById('add-category').value;
  var oldPrice = parseFloat(document.getElementById('add-old-price').value) || 0;
...
function updateDiscountCalc() {
  var pInput = document.getElementById('edit-price');
  var oInput = document.getElementById('edit-old-price');
  var preview = document.getElementById('edit-discount-preview');
  var valEl = document.getElementById('edit-discount-value');

  var price = parseFloat(pInput ? pInput.value : 0) || 0;
  var oldPrice = parseFloat(oInput ? oInput.value : 0) || 0;
